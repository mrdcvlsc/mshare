...
int main(){
	...
	//string   = fs::current_path().string();
	string guestDir		 =  "storage/shared/";
	string htmlTemplates =  "templates/";
	string htmlHead      = htmlTemplates + "head.txt",
		   htmlTail		 = htmlTemplates + "tail.txt",
		   htmlGuestFILE =  "views/guestview.ejs"; 
	...
	for(const auto& paths: fs::directory_iterator(guestDir)){
		...
		guestFiles.push_back(
			files(
				eraseSubStr(buffer.path().string(),  "/public"),
